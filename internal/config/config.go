...
type Config struct {
	...
	// Настроек для ВК здесь больше нет (были в в VKConfig до Р-016 в
	// docs/DECISIONS.md). Источник ВК — это расширение браузера
	// (extension/), а не Go-код: программе неоткуда взять cookie или порт
	// отладки, потому что она их больше не использует. См. docs/VK_ACCESS.md.

	// LastFM — настройки для обогащения жанрами через Last.fm (v0.7.0).
	LastFM LastFMConfig
}

// LastFMConfig — доступ к API Last.fm.
//
// Оба поля читаются как НЕобязательные (см. getString в Load, а не
// getRequired) — сознательное решение. Ключ нужен только команде
// обогащения жанрами. Если сделать его обязательным здесь, в Config,
// то сломается вообще любая команда программы — даже "music import file",
// которая про Last.fm ничего не знает и знать не должна — просто
// потому, что человек ещё не успел завести ключ. Обязательность нужно
// проверять там, где ключ реально используется (в самой команде
// обогащения), а не здесь, для всех сразу.
type LastFMConfig struct {
	// APIKey — ключ, который передаётся в каждый запрос к API.
	APIKey string

	// SharedSecret — нужен только для запросов, подписанных от имени
	// залогиненного пользователя (например, скробблинг). Обогащение
	// читает только публичные данные (теги исполнителя) и подписи не
	// требует, но секрет всё равно сохраняем — раз Last.fm его выдал
	// вместе с ключом, пусть будет под рукой, если понадобится позже.
	SharedSecret string
}

// MaskedAPIKey — версия ключа для журналов: видны только последние 4
// символа, остальное скрыто звёздочками. Тот же приём, что и SafeDSN
// у базы данных: секрет не должен целиком попадать в лог, даже свой.
// Если ключ короче 4 символов или вообще пуст — значит, скорее всего,
// он не задан, и скрывать нечего: возвращаем как есть, чтобы в логе
// было видно "пусто", а не сбивающие с толку звёздочки.
func (c LastFMConfig) MaskedAPIKey() string {
	if len(c.APIKey) <= 4 {
		return c.APIKey
	}
...
func Load() (*Config, error) {
	...
	cfg := &Config{
		...
		LastFM: LastFMConfig{
			// getString, не getRequired — см. комментарий у LastFMConfig,
			// почему отсутствие ключа не должно ронять вообще все команды.
			APIKey:       getString("LASTFM_API_KEY", ""),
			SharedSecret: getString("LASTFM_SHARED_SECRET", ""),
		},
	}
